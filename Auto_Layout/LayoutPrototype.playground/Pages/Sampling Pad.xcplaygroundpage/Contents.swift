...
let verticalStackView = VerticalStackView(arrangedSubviews:[horizontalStackView])
...
NSLayoutConstraint.activate([
    verticalStackView.leadingAnchor.constraint(equalTo: samplingPad.leadingAnchor,constant: horizontalSpacing),
    verticalStackView.topAnchor.constraint(equalTo: samplingPad.topAnchor, constant: verticalSpacing),
    verticalStackView.trailingAnchor.constraint(equalTo: samplingPad.trailingAnchor,constant: -horizontalSpacing),
    verticalStackView.bottomAnchor.constraint(equalTo: samplingPad.bottomAnchor,constant: -verticalSpacing),
])
...
let volumeControlsStackView = HorizontalStackView(arrangedSubviews: [volumeSlider,volumeButtonsStackView])
verticalStackView.insertArrangedSubview(volumeControlsStackView, at: 0)
volumeButtonsStackView.widthAnchor.constraint(equalToConstant: 120).isActive = true
samplingPad.layoutIfNeeded()
...
NSLayoutConstraint.activate( [leftSpacerView.widthAnchor.constraint(equalToConstant: 8),
...
volumeSlider.addTarget(samplingPad, action: #selector(SamplingPad.volumeSliderValueDidChange), for: .valueChanged)
...
[increaseVolumeButton, decreaseVolumeButton].forEach {
    $0.addTarget(
    samplingPad,
    action: #selector(SamplingPad.volumeButtonDidTouchUpInside(_:)), for: .touchUpInside) }
